    ...
    ): Response
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            foreach ($codes as $i => $code) {
                ...
                // Si un taux manque → fallback aux taux du jour
        ...
        // GET : formulaire + quotes de base (buy/sell) exposés au front pour le toggle Achat/Vente
        return $this->render('admin/invoice.html.twig', [
            ...
            'baseQuotes' => (function() use ($rates){
                ...
                foreach ($rates->getSupportedCurrencies() as $c) {
                    $code = $c['code'];
                    $r = $rates->computeBuySell($code); // ['buy'=>..., 'sell'=>...], sens EUR->Local
                    $out[$code] = [
